...
class ISLRDataset(torch.utils.data.Dataset):
    ...
    def load_keypoints(self):
        if 'keypoint' in self.input_streams or 'keypoint_coord' in self.input_streams or 'trajectory' in self.input_streams:
            ...
            for name, all_keypoints in name2all_keypoints.items():
                # Se il dato è un dizionario, estrai il valore associato a "keypoints"
    ...
    def __getitem__(self, idx):
        ...
        if(self.dataset_cfg['isContinuous'] and self.dataset_cfg['isContinuous']==True):
            name = data['name'] + ".mp4"
            print("++\\OUTPUT GETITEM//++")
            print("| ", {"name": name, "text": data['text'], "seq_len": data['length'], "label": data['text'], "video_file": name})
            print("|---------------------------------------------------------------------------")
